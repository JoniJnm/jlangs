...
function search_langs(
	$folderSearch,
	$exts,
	$funcNames,
	$fileOutput,
	$exc_folders = array(".git", 'node_modules', 'bower_components', 'lib', 'libs', 'vendor')
) {
	...
	foreach ($files as $file) {
		...
		preg_match_all("/[^a-zA-Z0-9\-_](?:" . implode('|', $funcNames) . ")\s?\((.*?(?:\"|'))\)/",
			file_get_contents($file), $matchs);
		if (!$matchs) {
			continue;
		}
...
function listAllFiles($baseFolder, $ops = array())
{
	...
	if ($level_min < 0 || ($level_max >= 0 && $level > $level_max)) {
		return array();
	}
